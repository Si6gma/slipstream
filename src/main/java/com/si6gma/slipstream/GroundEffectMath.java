...
public final class GroundEffectMath {
   ...
   *
   * <p>When descending, includes an antigravity term (~0.02/tick) that offsets elytra's residual
  ...
  public static double liftForce(
    ...
    // Antigravity: offsets elytra's ~0.02/tick residual gravity at level flight.
   ...
   * maxSpeed, or if the entity is ascending beyond the deadband; ground effect only accelerates
   * level or descending flight. The 0.05 deadband absorbs the ~0.02/tick antigravity noise so lift
   * nudging velocity.y just past zero does not toggle the gate every tick.
   */
